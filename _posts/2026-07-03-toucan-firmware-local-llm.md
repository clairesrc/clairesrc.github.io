...
That's the part I'm used to. The interesting choice this time was the harness.

## Why Oh My Pi instead of OpenCode

For coding work I normally reach for [OpenCode](https://opencode.ai), running on a home server and exposing a web UI so I can resume a session from any device over the LAN. But firmware has a physical loop: build the UF2, double-tap reset, drag the file onto the bootloader mount, test it on real hardware, repeat. That loop gets old fast when your agent lives on a server in another room and the keyboard is sitting on your desk.
